...
async function closeNonChangelogIssues(existingIssues) {
  let closed = 0;

  for (const issue of existingIssues) {
    const labels = (issue.labels ?? []).map((l) => l.name);
    if (labels.includes("changelog")) continue;
...
async function main() {
  ...
  // Close all open ai-updates issues that aren't weekly changelog issues
  console.log("\nClosing non-changelog ai-updates issues...");
  const issuesClosed = await closeNonChangelogIssues(existingIssues);
  console.log(`  Closed ${issuesClosed} issue(s).`);
  ...
  console.log(`\nDone. Closed ${issuesClosed} old issue(s), created weekly changelog issue.`);
}

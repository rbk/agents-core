...
These rules apply to every coding task, in every project, with every agent.
...
- **Always flag security design flaws.** If you notice an insecure pattern — an unprotected API endpoint, missing input validation, exposed secrets, broken auth — point it out immediately and offer concrete suggestions to fix it, even if it's outside the scope of the current task.
